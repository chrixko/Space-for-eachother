...
namespace LoveCommander {
	...
	public class KinectContext {
		...
		public void Initialize() {
			...
			this.CurrentBitmap = Assets.Images["glove"];
		}
		...
		public Point SkeletonPointToScreen(SkeletonPoint skelpoint) {            
			...
			return new Point((int)(colorPoint.X / MainGame.KinectScaleX), (int)(colorPoint.Y / MainGame.KinectScaleY));
		}
		...
		public Skeleton GetLeftSkeleton() {
			return (from Skeleton s in Skeletons where s != GetRightSkeleton() orderby s.Position.Z, s.Position.X ascending select s).FirstOrDefault();
		}

		public Skeleton GetRightSkeleton() {            
			return (from Skeleton s in Skeletons orderby s.Position.Z ascending orderby s.Position.X descending select s).FirstOrDefault();            
		}
